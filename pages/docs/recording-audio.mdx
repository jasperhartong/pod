...
The design might look slightly familiar, as it's a reimplementation from the [Apple© Voice Memos app](https://support.apple.com/en-us/HT206775) 😇.
...
Internally there are these brilliant "magic" values that just took some toying around for it to look nice for voice recordings 🧙‍♂️.
...
#### Fluent animations
* By using `requestAnimationFrame` to give the browser control about when there are enough resources to recalculate a frame of the animation.
* Instead of using React state to rerender, it's doing direct manipulation of the DOM and the style properties.
* By creating references towards the DOM nodes upon mounting, these style adjustments can be made without repeated look ups.
...
### ⚠️ MediaRecorder required a polyfill
...
### ⚠️ AudioContext required standardization
