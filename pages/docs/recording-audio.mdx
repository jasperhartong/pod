...
<Link href={"/docs"} as={"/docs"}>← Back to overview</Link>
...
Well, quite hard and véry frustrating, as Chrome, Safari and Firefox all have inconsistent Web Audio API's and recording behavior.
...
The design of the button might look slightly familiar, as it's a reimplementation from the [Apple© Voice Memos app](https://support.apple.com/en-us/HT206775). For me this is just a good excercise to force myself on the implementation of these type of micro interactions, instead of loosing myself in endless design iterations in these side-projects.

### 📓 Don't create custom UI from scratch

The `AudioRecorderButton` is based on the [ButtonBase of Material-UI](https://material-ui.com/api/button-base/). This ensures it correctly handles the easily overlooked basics of a button (e.g. focusability, accesability). This basic stuff is easy to get wrong when building completely from scratch.
...
Internally there are these brilliant "magic" values that just took some toying around for it to look nice for voice recordings 🧙‍♂️. While experimenting around it's easiest to move these values into props and create a simple playground for yourself.
...
For recording the hook also allows to define a time slice in millisecconds to save the mp3 data in chunks. When not setting a time slice all recording data is ony available after stopping the recording.
...
- Gaurds undesired state transitions (e.g. ignore `stopListening` when already recording)
- Teardowns all the different listeners and callbacks needed for listening and recording upon unmounting (so the browser no longer shows a red marker)
...
### 📓 Web Audio API's are a mess..

It took a lot of trials to settle on the internals of the hook that would work consistently accross different browsers. In the end I settled on the following combination of polyfills and standardization. This combination now seems to work everywhere, well.. almost everywhere.. somehow, when using Airpods on Mobile Safari it breaks in a weird way, quite a buzz-kill.
...
As it's a side-project and was mostly for me to fiddle around, most components are NOT as clean as I would like. Don't expect there to be any tests neither 🤷‍♂️. The code might be usefull as inpsiration though!
